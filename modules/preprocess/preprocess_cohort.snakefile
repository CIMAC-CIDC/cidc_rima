...
rule STAR_matrix:
    ...
    shell:
      """{params.path}; perl cidc_rima/src/preprocess/STAR_reports.pl -f {params.log_files} 1>{output.csv} """
...
rule read_distrib_qc_matrix:
    ...
    params:
      ...
      path="set +eu;source activate %s" % config['stat_root'],
    shell:
      "perl cidc_rima/src/preprocess/read_distrib_matrix.pl -f {params.file_list_with_flag} 1>{output.matrix} "
...
rule salmon_matrix:
    ...
    params:
      ...
      tx2gene = 'cidc_rima/static/deseq2/tx2gene.csv',
      outpath = 'analysis/salmon/',
      path = "set +eu;source activate %s" % config['stat_root']
      
    message: "Merge Salmon gene quantification together for all samples "
    shell:
      "{params.path}; Rscript cidc_rima/src/preprocess/merge_tpm.R --input {params.args} --meta {input.metasheet} \
...
rule batch_removal:
    ...
    conda: "../envs/stat_perl_r.yml"
    shell:
        "{params.path}; Rscript cidc_rima/src/preprocess/batch_removal.R -e {input} -c {params.covariates} \
...
rule pca_sample_clustering:
    ...
    conda: "../envs/stat_perl_r.yml"
    shell:
        "{params.path}; Rscript cidc_rima/src/preprocess/pca.R -b {input.before_batch} -a {input.after_batch} -m {params.meta_info}  -c {params.covariates} \
